...
export function Controller(prefix?: string): ClassDecorator {
    const routePrefix =
        prefix == "/" || prefix === undefined ? "" : CommonUtil.cutRoutePath(prefix);
...
function createRouteMethodDecorator(method: RouteMethod) {
    return (path?: string, ...middlewares: MiddlewareFunc[]): MethodDecorator => {
        return (target: object, key: string | symbol, descriptor: PropertyDescriptor) => {
            path = path == "" || path === undefined ? "/" : CommonUtil.cutRoutePath(path);
...
function createRouteParamDecorator(metadataKey: string) {
    return (name?: string): ParameterDecorator => {
        return (target: object, key: string | symbol | undefined, index: number) => {
            ...
            MetadataUtil.extendArray(metadataKey, paramMetadata, target.constructor, key);
        };
    };
}
