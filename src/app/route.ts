...
import { PropertyArrayMetadata, PropertyMetadata } from "@/common/decorators/parseDecorator";
import {
    ClassMetadata,
    CtxMetadata,
    ParamMetadata,
...
export class Route {
    ...
    async initialize() {
        for (const classMetadata of this.objectsFactory.getClassMetadataContainer().values()) {
            ...
            for (const [methodName, methodArgs] of classMetadata.methodsArgsMap) {
                ...
                this.engine.use(
                    CommonUtil.cutRoutePath(configService.apiPrefix) +
                        routeClassMetadata.routePrefix,
                    router,
                );

                if (configService.app?.printBindRoutesLog) {
                    loggerService.debug(
                        `${
        ...
        loggerService.info(`Routes initialized.`);
    }
    ...
    private async initializeHandler(options: InitializeHandlerOptions) {
        ...
        return async (req: Req, res: Res, next: Next) => {
            const requestId = CommonUtil.generateRequestId();
    ...
    private async assignMethodArgsToInstancesAndValidate(options: AssignMethodArgsOptions) {
        ...
        for (const [routeKey, routeParamKey] of this.routeParams) {
            ...
            for (const paramMetadata of paramsMetadata) {
                ...
                if (ObjectsUtil.isObject(routeParamValue)) {
    ...
    private async checkForAuthorization(
        ...
        const payload = jwtService.parse(token);
        if (payload === null) {
        ...
        const userToken = await cacheService.get(
            CommonUtil.generateCacheKey(KEY_USER_TOKEN, userId),
        );
        if (userToken === null) {
    ...
    private async checkForAuthentication(
        ...
        const roleRecords = await dataSourceService
            .getRepository(Role)
        ...
        const permissionRecords = await dataSourceService
